...
NNIE::NNIE() {}
NNIE::~NNIE() { NNIE_Param_Deinit(&s_stNnieParam_, &s_stModel_); }
void NNIE::init(const char *model_path, const int image_height, const int image_width)
{
...
void NNIE::run(const unsigned char *data) { NNIE_Forward_From_Data(data, &s_stModel_, &s_stNnieParam_, output_tensors_); }
...
void NNIE::finish() { NNIE_Param_Deinit(&s_stNnieParam_, &s_stModel_); }

Tensor NNIE::getOutputTensor(int index) { return output_tensors_[index]; }
